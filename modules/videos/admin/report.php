...
$page_title = $nv_Lang->getModule('videos_reports');
...
if ($num > 0) {
    $array_reports = array(
        1 => $nv_Lang->getModule('report_notplay'),
        2 => $nv_Lang->getModule('report_content'),
        3 => $nv_Lang->getModule('report_copyright'),
        4 => $nv_Lang->getModule('report_other')
    );
    ...
    while ($data = $result->fetch()) {
        ...
        ++ $ae;
        $data['num'] = $ae;
        $data['delete'] = "<em class=\"fa fa-trash-o fa-lg\">&nbsp;</em> <a href=\"javascript:void(0);\" onclick=\"nv_del_report('" . $data['rid'] . "', '" . $data['id'] . "', '" . $data['checkss'] . "')\">" . $nv_Lang->getModule('report_del') . "</a>";
        $data['edit'] = '<em class="fa fa-edit margin-right">&nbsp;</em> <a href="' . $data['edit_link'] . '">' . $nv_Lang->getModule('content_edit') . '</a>';
