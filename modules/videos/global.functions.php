...
function nv_link_edit_page($id)
{
    ...
    $link = "<a class=\"btn btn-primary btn-xs\" href=\"" . NV_BASE_ADMINURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=content&amp;id=" . $id . "\"><em class=\"fa fa-edit margin-right\"></em> " . $nv_Lang->getGlobal('edit') . "</a>";
    return $link;
}
...
{
    global $lang_global, $module_name,$nv_Lang;
    $link = "<a class=\"btn btn-danger btn-xs\" href=\"javascript:void(0);\" onclick=\"nv_del_content(" . $id . ", '" . md5($id . session_id()) . "','" . NV_BASE_ADMINURL . "', " . $detail . ")\"><em class=\"fa fa-trash-o margin-right\"></em> " . $nv_Lang->getGlobal('delete') . "</a>";
    return $link;
}
...
function nv_link_edit_playlist($id)
{
    ...
    $link = "<a class=\"btn btn-primary btn-xs\" href=\"" . NV_BASE_ADMINURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=playlists&amp;playlist_id=" . $id . "#edit\"><em class=\"fa fa-edit margin-right\"></em> " . $nv_Lang->getGlobal('edit') . "</a>";
    return $link;
}
...
{
    global $lang_module,$nv_Lang;
    
    $time = time() - $time; // to get the time since that moment
    $time = ($time < 1) ? 1 : $time;
    $tokens = array(
        31536000 => $nv_Lang->getModule('year'),
        2592000 => $nv_Lang->getModule('month'),
        604800 => $nv_Lang->getModule('week'),
        86400 => $nv_Lang->getModule('day'),
        3600 => $nv_Lang->getModule('hour'),
        60 => $nv_Lang->getModule('minute'),
        1 => $nv_Lang->getModule('second')
    );
...
function nv_videos_getuser_info($user_id)
{
    ...
    while ($uploader_info = $result->fetch()) {
        ...
        $_des = $nv_Lang->getModule('about_uploader') . $uploader_info['username'];
